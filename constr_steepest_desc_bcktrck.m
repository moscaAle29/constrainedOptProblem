function [xk, fk, gradfk_norm, deltaxk_norm, k, xseq, btseq, piseq] = ...
    ...
    kmax, tolgrad, c1, rho, btmax, gamma, tolx, feasible_set, ...
    findiff_enable, findiff_k,method)
%
...
xk = box_projection(x0,feasible_set); % Project the starting point if outside the constraints
fk = f(xk);
if findiff_enable
    gradfk = findiff_grad(xk,get_h(xk,findiff_k),method);
else
    gradfk = gradf(xk);
end
